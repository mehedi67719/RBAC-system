...
import { Search, Filter, Shield, User, Users, Briefcase, Save, AlertCircle, ChevronDown, ChevronRight, Check, X } from 'lucide-react';
    ...
    <div className="p-6">
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage user permissions and access control</p>
        </div>
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6">
        <div className="flex items-start space-x-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-blue-900">Grant Ceiling Enforcement</h3>
            <p className="text-xs text-blue-700 mt-1">
              You can only assign permissions you hold yourself. Permissions are inherited from your role and cannot exceed your own access level.
            </p>
          </div>
        </div>
      </div>
                          ...
                          {category.atoms.map((atom) => (
                            <label key={atom.id} className="flex items-start space-x-2 cursor-pointer group">
                              ...
                              <div className="flex-1">
                                <p className="text-sm font-medium text-gray-700 group-hover:text-gray-900">{atom.name}</p>
                                <p className="text-xs text-gray-400">{atom.description}</p>
                              </div>
                            </label>
                          ))}
                        </div>
